...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'scientia_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'scientia_trx_addons_theme_specific_components' );
	function scientia_trx_addons_theme_specific_components( $enable = false ) {
		return false;
	}
}

if ( ! function_exists( 'scientia_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'scientia_trx_addons_theme_specific_setup1', 1 );
	function scientia_trx_addons_theme_specific_setup1() {
		if ( scientia_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'scientia_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'scientia_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'scientia_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'scientia_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'scientia_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'scientia_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'scientia_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'scientia_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'scientia_trx_addons_theme_specific_setup' );
	function scientia_trx_addons_theme_specific_setup() {
		if ( scientia_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'scientia_action_before_post_meta', 'scientia_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'scientia_trx_addons_cv_enable' ) ) {
	
	function scientia_trx_addons_cv_enable( $enable = false ) {
		// To do: return false if theme not use CV functionality
		return SCIENTIA_THEME_FREE
					? false     // Free version
					: true;     // Pro version
	}
}
...
// Demo mode
if ( ! function_exists( 'scientia_trx_addons_demo_enable' ) ) {
	
	function scientia_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'scientia_trx_addons_edd_themes_market_enable' ) ) {
	
	function scientia_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'scientia_trx_addons_api_list' ) ) {
	
	function scientia_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'scientia_trx_addons_cpt_list' ) ) {
	
	function scientia_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'scientia_trx_addons_sc_list' ) ) {
	
	function scientia_trx_addons_sc_list( $list = array() ) {
		...
		$list['blogger']['templates']['default']['simple_grid'] = array(
            'title'  => esc_html__('Simple Grid', 'scientia'),
            'layout' => array(
                'content' => array(
                    'title', 'excerpt', 'meta'
                )
		...
		$list['blogger']['templates']['default']['vertical_grid'] = array(
            'title'  => esc_html__('Vertical Grid', 'scientia'),
            'layout' => array(
                'featured' => array(
                    'tl' => array(
                        'meta', 'title'
...
// Widgets
if ( ! function_exists( 'scientia_trx_addons_widgets_list' ) ) {
	
	function scientia_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'scientia_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'scientia_trx_addons_menu_cache' );
	function scientia_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'scientia_trx_addons_localize_script' ) ) {
	...
	function scientia_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = scientia_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'scientia_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'scientia_trx_addons_sc_type', 10, 2 );
	function scientia_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = scientia_array_merge( $list, scientia_get_list_blog_styles( false, 'sc' ) );
		}

		// adding new type of icons
		if ( 'trx_sc_icons' == $sc ) {
            $list['red'] = esc_html__('Red', 'scientia');
        }
...
if ( ! function_exists( 'scientia_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'scientia_trx_addons_sc_prepare_atts', 10, 2 );
	function scientia_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = scientia_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = scientia_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'scientia_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'scientia_trx_addons_sc_atts', 10, 2 );
	function scientia_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'scientia_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'scientia_trx_addons_sc_map', 10, 2 );
	function scientia_trx_addons_sc_map( $params, $sc ) {
		...
		if ( in_array(
			...
			$params['params'][] = array(
				'param_name'  => 'scheme',
				'heading'     => esc_html__( 'Color scheme', 'scientia' ),
				'description' => wp_kses_data( __( 'Select color scheme to decorate this block', 'scientia' ) ),
				'group'       => esc_html__( 'Colors', 'scientia' ),
				'admin_label' => true,
				'value'       => array_flip( scientia_get_list_schemes( true ) ),
				'type'        => 'dropdown',
			);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'scientia' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'scientia' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( scientia_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'scientia' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'scientia' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( scientia_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'scientia_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'scientia_trx_addons_sc_output', 10, 4 );
	function scientia_trx_addons_sc_output( $output, $sc, $atts, $content ) {

		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			if ( ! empty( $atts['scheme'] ) && ! scientia_is_inherit( $atts['scheme'] ) ) {
				$output = str_replace( 'class="sc_action ', 'class="sc_action scheme_' . esc_attr( $atts['scheme'] ) . ' ', $output );
			}
			if ( ! empty( $atts['color_style'] ) && ! scientia_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'scientia_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'scientia_trx_addons_sc_item_link_classes', 10, 3 );
	function scientia_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! scientia_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'scientia_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'scientia_trx_addons_sc_item_title_tag' );
	function scientia_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'scientia_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'scientia_trx_addons_sc_item_button_args', 10, 3 );
	function scientia_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'scientia_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'scientia_trx_addons_slider_title', 10, 2 );
	function scientia_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'scientia_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'scientia_trx_addons_sc_googlemap_styles' );
	function scientia_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'scientia' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'scientia_trx_addons_action_before_post_meta' ) ) {
	
	function scientia_trx_addons_action_before_post_meta() {
		if ( scientia_exists_trx_addons() ) {
...
if ( ! function_exists( 'scientia_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'scientia_trx_addons_tiny_mce_style_formats' );
	function scientia_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'scientia' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'scientia' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'scientia_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'scientia_filter_allow_override_header_image', 'scientia_trx_addons_allow_override_header_image' );
	function scientia_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					scientia_is_team_page()
					|| scientia_is_cars_page()
					|| scientia_is_cars_agents_page()
					|| scientia_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'scientia_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'scientia_trx_addons_thumb_size', 10, 2 );
	function scientia_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
