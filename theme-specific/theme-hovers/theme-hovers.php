...
if ( ! function_exists( 'scientia_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'scientia_hovers_theme_setup3', 3 );
	function scientia_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		scientia_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'scientia' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'scientia' ) ),
					'std'     => 'slide_left',
					'options' => array(
						'slide_left'   => esc_html__( 'Slide from Left', 'scientia' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'scientia' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'scientia' ) ),
					'std'      => 'dots',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'scientia' ),
					),
					'options'  => scientia_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'scientia_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'scientia_hovers_theme_setup9', 9 );
	function scientia_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'scientia_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'scientia_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'scientia_hovers_responsive_styles', 2000 );
		add_filter( 'scientia_filter_localize_script', 'scientia_hovers_localize_script' );
		add_filter( 'scientia_filter_merge_scripts', 'scientia_hovers_merge_scripts' );
		add_filter( 'scientia_filter_merge_styles', 'scientia_hovers_merge_styles' );
		add_filter( 'scientia_filter_merge_styles_responsive', 'scientia_hovers_merge_styles_responsive' );
		add_filter( 'scientia_filter_get_css', 'scientia_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'scientia_hovers_frontend_scripts' ) ) {
	
	function scientia_hovers_frontend_scripts() {
		if ( scientia_is_on( scientia_get_theme_option( 'debug_mode' ) ) ) {
			$scientia_url = scientia_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $scientia_url ) {
				wp_enqueue_script( 'scientia-hovers', $scientia_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'scientia_hovers_frontend_styles' ) ) {
	
	function scientia_hovers_frontend_styles() {
		if ( scientia_is_on( scientia_get_theme_option( 'debug_mode' ) ) ) {
			$scientia_url = scientia_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $scientia_url ) {
				wp_enqueue_style( 'scientia-hovers', $scientia_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'scientia_hovers_responsive_styles' ) ) {
	
	function scientia_hovers_responsive_styles() {
		if ( scientia_is_on( scientia_get_theme_option( 'debug_mode' ) ) ) {
			$scientia_url = scientia_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $scientia_url ) {
				wp_enqueue_style( 'scientia-hovers-responsive', $scientia_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'scientia_hovers_merge_styles' ) ) {
	
	function scientia_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'scientia_hovers_merge_styles_responsive' ) ) {
	
	function scientia_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'scientia_hovers_localize_script' ) ) {
	
	function scientia_hovers_localize_script( $arr ) {
		$arr['button_hover'] = scientia_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'scientia_hovers_merge_scripts' ) ) {
	
	function scientia_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'scientia_hovers_add_icons' ) ) {
	function scientia_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php scientia_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'scientia' ) : esc_html__( 'Buy now', 'scientia' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'scientia' );
					} else {
						esc_html_e( 'Details', 'scientia' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php scientia_show_layout($target); ?> aria-hidden="true" class="icon-01-book"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php scientia_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php scientia_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php scientia_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'scientia_hovers_get_css' ) ) {
	
	function scientia_hovers_get_css( $css, $args ) {
