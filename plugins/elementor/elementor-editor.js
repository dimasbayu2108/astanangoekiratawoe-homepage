...
jQuery( document ).ready(
	function() {
		...
		scientia_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="scientia_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.scientia_post_editor' );
					if ( link.length > 0 ) {
						scientia_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.scientia_post_editor', function(e) {
					scientia_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('scientia_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function scientia_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
