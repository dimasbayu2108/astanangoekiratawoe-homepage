<div class="front_page_section front_page_section_features<?php
	$scientia_scheme = scientia_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $scientia_scheme ) && ! scientia_is_inherit( $scientia_scheme ) ) {
		echo ' scheme_' . esc_attr( $scientia_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( scientia_get_theme_option( 'front_page_features_paddings' ) );
	if ( scientia_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$scientia_css      = '';
		$scientia_bg_image = scientia_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$scientia_anchor_icon = scientia_get_theme_option( 'front_page_features_anchor_icon' );
	$scientia_anchor_text = scientia_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( scientia_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$scientia_css      = '';
			$scientia_bg_mask  = scientia_get_theme_option( 'front_page_features_bg_mask' );
			$scientia_bg_color_type = scientia_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $scientia_bg_color_type ) {
				$scientia_bg_color = scientia_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$scientia_caption = scientia_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $scientia_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $scientia_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $scientia_caption, 'scientia_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$scientia_description = scientia_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $scientia_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $scientia_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $scientia_description ), 'scientia_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
				...
				if ( ! scientia_exists_trx_addons() ) {
					...
					scientia_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
				}
			}
			?>
			</div>
